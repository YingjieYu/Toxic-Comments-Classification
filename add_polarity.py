...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 21 23:30:38 2018
...
nrow=train.shape[0]
#%%
df['polarity'] = train['comment_text'].map(lambda x: int(TextBlob(x).sentiment.polarity *10))
df['comment_text'] = df.apply(lambda r: str(r['comment_text']) + 'polarity' + zsign[np.sign(r['polarity'])]+zpolarity[np.abs(r['polarity'])], axis =1)
